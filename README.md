...
| 스킬 | 하는 일 | 연관 스킬 |
|---|---|---|
| `project-struct` | 프로젝트 구조/컨벤션/API 카탈로그/외부연동 맵을 분석해서 `docs/STRUCTURE.md` 등을 생성 | 다른 스킬들이 배경지식으로 공통 참고 |
| `5w1h` | 기존 프로젝트/기능을 추가개발·유지보수·버그수정·접근방법결정·고려사항 관점으로 분석 | `project-struct` 없으면 먼저 실행 → `backend-dev`의 필수 입력 |
| `backend-dev` | 5w1h 분석 문서 기반으로 실제 백엔드 코드 구현 | `5w1h` 필수, `project-struct` 컨벤션 활용, `code-review` 자동 호출, `test-case-creator` 요청 시 호출 |
| `code-review` | git diff(현재 브랜치=미커밋 포함) 리뷰, Critical/Medium/Low 분류 + 호출부·사이드이펙트 검증 | `backend-dev`가 자동 호출(Critical/Medium 자동 반영) / 독립 호출 시 항상 확인 후 반영 |
| `test-case-creator` | git 브랜치 diff 기반 4관점(기능/보안성/안정성/회귀) 테스트케이스 및 코드 생성 | `backend-dev`가 테스트 요청 시에만 호출 / 독립 호출 가능 |
| `meeting-speech-review` | 회의/발표 대본을 5W1H 완결성 + 구어체 어순으로 검토 | `5w1h` 문서 있으면 참고(자동 연결은 없음, 항상 직접 호출) |
...
```
project-struct ──(배경지식)──▶ 5w1h ──(필수 재료)──▶ backend-dev
                     ▲                                    │
                     │ (참고만, 자동연결 없음)               ├─(자동, 항상)──▶ code-review
                     └── meeting-speech-review              │                     ▲
                                                             └─(요청시만)──▶ test-case-creator
                                                                                   │
                                                             (독립 호출도 가능) ◀───┘
```
